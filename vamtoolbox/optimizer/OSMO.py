...
def minimizeOSMO(target_geo,proj_geo,options, **kwargs):
    ...
    def stepVoid(x,prev_xm):
        ...
        if options.bit_depth is not None:
            b = vamtoolbox.util.data.discretize(b,options.bit_depth,[np.min(b),np.max(b)])
    ...
    target_filtered = vamtoolbox.util.data.filterTargetOSMO(target_geo.array,options.filter)
    ...
    if options.verbose == 'plot':
        dp.ioff()
    # plt.close()
    return vamtoolbox.geometry.Sinogram(b_opt,proj_geo,options), vamtoolbox.geometry.Reconstruction(x,proj_geo,options), _error
